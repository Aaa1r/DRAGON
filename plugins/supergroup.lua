...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '☑️ تہۨم تہۨفہۨعہۨيہۨل الہۨمہۨجہۨموعة ☑️️.'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '☑️ تہۨم تہۨعہۨطہۨيہۨل الہۨمہۨجہۨموعة ☑️️.'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return '😠تہۨم قہۨفہۨل الہۨكہۨلايہۨش 😼✋'
  else
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return '😌تہۨم فہۨتہۨح🔓 الہۨكلايہۨش 😸👏'
  else
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'تفعيل المجموعه' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, '☑️الہۨمہۨجہۨموعة بالہۨتاكہۨيد مہۨفہۨعہلہۨه☑️️..', ok_cb, false)
			end
		...
		if matches[1] == 'تعطيل المجموعه' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, '☑️الہۨمہۨجہۨموعة بالہۨتاكہۨيد تہۨم تہۨعہۨطہۨيلها☑️️..', ok_cb, false)
			end
		...
		if matches[1] == 'قفل' and is_momod(msg) then
			...
			local msg_type = 'Audio'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return msg_type.."😋تْمْ قْفْلْ🔒آلْصْوْتْ😏🖖🏻"
				else
			...
			local msg_type = 'Photo'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.."😋تمْ قْفْلْ🔒آلْصْوْرْ😏🖖🏻"
				else
					return "SuperGroup mute "..msg_type.." is already on"
				end
			end
			...
			local msg_type = 'Video'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.."😋تْمْ قْفْلْ 🔒آلْفْيْدْيْوْ😏🖖🏻"
				else
					return "SuperGroup mute "..msg_type.." is already on"
				end
			end
			...
			local msg_type = 'Gifs'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.."😋تْمْ قْفْلْ🔒آلْصْوْرْ آلْمْتْحْرْڪْﮬ̲̣̐😏🖖🏻"
				else
					return "SuperGroup mute "..msg_type.." is already on"
				end
			end
			...
			local msg_type = 'Documents'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.."😋تْمْ قْفْلْ🔒آلْفْآيْلْآتْ😏🖖🏻"
				else
					return "SuperGroup mute "..msg_type.." is already on"
				end
			end
			...
			local msg_type = 'Text'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.."😋تْمْ قْفْلْ🔒آلْدْرْدْشْﮬ̲̣̐😏🖖🏻"
				else
		...
		if matches[1] == 'فتح' and is_momod(msg) then
			...
			local msg_type = 'Audio'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return msg_type.."😋تْمْ فْتْحْ🔓آلْصْوْتْ😌🖖🏻"
				else
			...
			local msg_type = 'Photo'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return msg_type.."😋تمْ فْتْحْ🔓آلْصْوْرْ😌🖖🏻"
				else
					return "Mute "..msg_type.." is already off"
				end
			end
			...
			local msg_type = 'Video'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return msg_type.."😋تْمْ فْتْحْ🔓آلْفْيْدْيْوْ😌🖖🏻"
				else
					return "Mute "..msg_type.." is already off"
				end
			end
			...
			local msg_type = 'Gifs'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return msg_type.."😋تْمْ فْتْحْ🔓آلْصْوْرْ آلْمْتْحْرْڪْﮬ̲̣̐😌🖖🏻"
				else
					return "Mute "..msg_type.." is already off"
				end
			end
			...
			local msg_type = 'Documents'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return msg_type.."😋تْمْ فْتْحْ🔓آلْفْآيْلْآتْ😌🖖🏻"
				else
					return "Mute "..msg_type.." is already off"
				end
			end
			...
			local msg_type = 'Text'
				if is_muted(chat_id, msg_type..': yes') then
					...
					unmute(chat_id, msg_type)
					return msg_type.."😋تمْ فْتْحْ🔓آلْدْرْدْشْﮬ̲̐😌🖖🏻"
				else
